...
{/* @example-description: Базовый `UsersStack` с аватарами пользователей и сопровождающим текстом. */}
<Playground>
  ```jsx
  <UsersStack
    photos={[
...
{/* @example-description: Пример `UsersStack` с `avatarsPosition="block-start"` и счётчиком участников. */}
<Playground>
  ```jsx
  <UsersStack
...
{/* @example-description: Кастомные обёртки аватаров в `UsersStack` с `Tooltip` и интерактивным элементом. */}
<Playground>
  ```jsx
  <UsersStack
    size="l"
...
| --- | --- | --- | --- |
| `avatarsPosition` | `"inline-start" \| "block-start" \| "inline-end"` | `inline-start` | Определяет положение аватаров Режим `block-start` рекомендуется использовать с размером `m`. |
| `count` | `number` | `Math.max(0, photos.length - visibleCount)` | Число, которое будет указано в счетчике. По умолчанию высчитывается по формуле `photos.length - visibleCount`. Если число больше 99, то счетчик скроется. |
| `getRootRef` | `Ref<HTMLDivElement>` | `-` |  |
| `photos` | `string[] \| UsersStackPhoto[]` | `[]` | Массив ссылок на фотографии либо массив структур типа `UsersStackPhoto`. |
| `size` | `"s" \| "m" \| "l"` | `m` | Размер аватарок. |
| `visibleCount` | `number` | `3` | Количество аватарок, которые будут показаны. Если в массиве `photos` больше элементов и не используется размер `s`, то будет показано количество остальных элементов. |
