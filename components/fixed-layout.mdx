...
> </Box>
> ```
> Так как элемент с `position="sticky"` находится в основном потоке документа, при миграции с `<FixedLayout vertical="bottom" />`
> важно также соблюсти порядок элемента в документе.
...
{/* @example-description: Устаревший `FixedLayout` с закреплением поиска в верхней части области. */}
<Playground Wrapper={FixedLayoutWrapper}>
  ...
  <Group mode="plain">
    ...
    <Spacing size={40} />
    <Box paddingInline={16} paddingBlock={12} maxBlockSize={170} overflow="auto" tabIndex={0}>
      <Paragraph>
...
{/* @example-description: Пример миграции с `FixedLayout` на `Box` с `position="sticky"`. */}
<Playground Wrapper={FixedLayoutWrapper}>
  ```jsx
  <Group mode="plain">
    <Box position="sticky" insetBlockStart={0}>
...
| `filled` | `boolean` | `-` | Это свойство определяет, будет ли фон компонента окрашен в цвет фона контента. Это часто необходимо для фиксированных кнопок в нижней части экрана. |
| `getRootRef` | `Ref<HTMLDivElement>` | `-` |  |
| `useParentWidth` | `boolean` | `-` | Всегда соответствовать ширине родителя. Ширина пересчитывается по событию `window` `resize`. |
| `vertical` | `"top" \| "bottom"` | `-` | Положение по вертикали, либо сверху, либо снизу. |
