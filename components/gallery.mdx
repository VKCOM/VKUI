...
{/* @example-description: Базовая галерея со слайдами фиксированной ширины и центрированием активного слайда. */}
...
{/* @example-description: Автоматическая смена слайдов в `Gallery` через свойство `timeout`. */}
<Playground Wrapper={BlockWrapper}>
  ```jsx
...
{/* @example-description: Зацикленная галерея `looped` с буллетами и стрелками навигации. */}
<Playground Wrapper={BlockWrapper}>
  ```jsx
...
{/* @example-description: Кастомизация стрелок `Gallery` через `arrowSize` и `arrowAreaHeight`. */}
<Playground Wrapper={BlockWrapper}>
  ```jsx
...
{/* @example-description: Включенный индикатор прогресса `bullets` для навигации по слайдам. */}
<Playground Wrapper={BlockWrapper}>
  ```jsx
...
{/* @example-description: Контролируемая галерея с внешним управлением текущим слайдом. */}
<Playground Wrapper={BlockWrapper}>
...
{/* @example-description: Галерея слайдов произвольной ширины через `slideWidth="custom"`. */}
<Playground Wrapper={BlockWrapper}>
  ```jsx
...
{/* @example-description: Настройка параметров анимации смены слайдов через `animationDuration` и `animationEasing`. */}
<Playground Wrapper={BlockWrapper}>
  ```jsx
  <Gallery
    slideWidth="90%"
...
| --- | --- | --- | --- |
| `align` | `AlignType` | `-` |  |
| `animationDuration` | `number` | `-` | Длительность анимации смены слайда в миллисекундах. |
| `animationEasing` | `PredefinedEasingType \| CubicBezierEasingType` | `-` | Функция для анимации.  Принимает одно из предопределённых значений или параметры функции [cubic-bezier](https://developer.mozilla.org/en-US/docs/Web/CSS/easing-function/cubic-bezier). |
| `arrowAreaHeight` | `"stretch" \| "fit"` | `-` | Размер активной зоны стрелок (в пикселях). Определяет область вокруг стрелок, реагирующую на взаимодействие пользователя. В дизайне свойство называется `arrowArea`. |
| `arrowNextLabel` | `string` | `-` | Текст для кнопки-стрелки вправо (вперед). Делает ее доступной для ассистивных технологий. |
| `arrowPrevLabel` | `string` | `-` | Текст для кнопки-стрелки влево (назад). Делает ее доступной для ассистивных технологий. |
| `arrowSize` | `"s" \| "m"` | `-` |  |
| `bullets` | `false \| "dark" \| "light"` | `-` |  |
| `bulletTestId` | `((index: number, active: boolean) => string)` | `-` | Передает атрибут `data-testid` для bullets. |
| `dragDisabled` | `boolean` | `-` | Позволяет отключить реагирование на тач-события. |
| `getRef` | `Ref<HTMLElement>` | `-` |  |
| `getRootRef` | `Ref<HTMLDivElement>` | `-` |  |
| `hasPointer` | `boolean` | `-` |  |
| `initialSlideIndex` | `number` | `0` | Индекс слайда по умолчанию. |
| `looped` | `boolean` | `-` | Включает зацикленность слайдов. |
| `nextArrowTestId` | `string` | `-` | Передает атрибут `data-testid` для кнопки перехода к следующему слайду. |
| `onChange` | `((current: number) => void)` | `-` |  |
| `onDragEnd` | `((e: CustomTouchEvent, targetIndex: number) => void) \| undefined` | `-` |  |
| `onDragStart` | `CustomTouchEventHandler` | `-` |  |
| `onNextClick` | `((event: MouseEvent<Element, MouseEvent>) => void)` | `-` | Будет вызвано при нажатии на кнопку-стрелку вправо. |
| `onPrevClick` | `((event: MouseEvent<Element, MouseEvent>) => void)` | `-` | Будет вызвано при нажатии на кнопку-стрелку влево. |
| `prevArrowTestId` | `string` | `-` | Передает атрибут `data-testid` для кнопки перехода к предыдущему слайду. |
| `resizeSource` | `"window" \| "element"` | `-` | Тип источника для отслеживания размера: - `window`: пересчет позиции слайдов будет происходить при изменении размеров `window` - `element`: пересчет позиции слайдов будет происходить при изменении размеров компонента. |
| `showArrows` | `boolean` | `-` |  |
| `slideIndex` | `number` | `-` |  |
| `slideLabel` | `string \| ((index: number, slidesCount: number) => string)` | `-` | Текст для слайда. Делает его доступным для ассистивных технологий. Может быть функцией. По умолчанию устанавливает `aria-label` вида `${Номер слайда} из ${Количества слайдов}`. |
| `slideRoleDescription` | `string` | `-` | Описание роли для слайда, для лучше понимания пользователей скринридеров. По умолчанию - `Слайд`. |
| `slideTestId` | `((index: number) => string)` | `-` | Передает атрибут `data-testid` для слайда. |
| `slideWidth` | `number \| LiteralUnion<"custom", string>` | `-` | Размер слайда.  Значение `"custom"` используется, если ширина у слайдов разная. |
| `timeout` | `number` | `0` | При передаче происходит автоматический переход к следующему слайду через переданное время в ms. Автоматический переход будет ставиться на паузу при: - начале свайпа для переключения слайда; - фокусе на любой элемент внутри компонента; - при наведении мыши на компонент. |
