...
То есть разработчики могут вручную сгенерировать свой набор значений токенов Appearance и тем
самым перекрасить интерфейс в свои цвета.
...
Заметьте, что в `vkui.css` сохранились значения токенов Appearance, а значений vkui-tokens нет, есть
...
Пример использования для Appearance остаётся прежним. Для того чтобы использовать VKUI и vkui-tokens
...
ReactDOM.render(
  ...
  <ConfigProvider platform="vkcom" hasNewTokens>
    <AppRoot>
      <Button />
    </AppRoot>
  </ConfigProvider>,
...
import { AppRoot, ConfigProvider, Button } from "@vkontakte/vkui";
import "@vkontakte/vkui/dist/vkui.css";
import "path/to/my/vkui-tokens.css";
