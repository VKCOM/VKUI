...
- **Не используй CI-режим** (команды `test:e2e:ci`, `test:e2e-update:ci`) — запуск без Docker не даёт воспроизводимых результатов, всегда запускай через Docker-режим
