...
describe(defaultFilterFn, () => {
  ...
  it("ignores quotes", () => {
    expect(
      defaultFilterFn("петер", { label: 'Санкт-"Петербург"' })
    ).toBeTruthy();
  });

  it("ignores braces", () => {
    expect(
      defaultFilterFn("пите", { label: "Санкт-Петербург (Питер)" })
    ).toBeTruthy();
  });

  it("search from the middle of sentence", () => {
    expect(
      defaultFilterFn("имя нельз", { label: "Группа, чьё имя нельзя называть" })
    ).toBeTruthy();
    expect(
