...
describe('touch', () => {
  describe.each(['coordX', 'coordY'] as const)('%s', (getterName) => {
    ...
    it('works with mouse', () => {
      expect(getter(new MouseEvent('mousemove', { clientX: 100, clientY: 100 }) as VKUITouchEvent)).toBe(100);
    });
    it('works with touch', () => {
      expect(getter(new TouchEvent('touchmove', { changedTouches: [touch({ clientX: 100, clientY: 100 })] }) as VKUITouchEvent)).toBe(100);
    });
    it('works with mouse when client=0', () => {
      expect(getter(new MouseEvent('mousemove', { clientX: 0, clientY: 0 }) as VKUITouchEvent)).toBe(0);
    });
  });
});
