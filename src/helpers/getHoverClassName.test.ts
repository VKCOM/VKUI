...
describe(getHoverClassName, () => {
  ...
  describe("with `styles` argument", () => {
    ...
    it("returns 'none' className if there is no hover", () =>
      expect(getHoverClassName("base", undefined, styles)).toBe(
        styles["base--hover-none"]
      ));

    it("returns className with 'has' if hover is true", () =>
      expect(getHoverClassName("base", true, styles)).toBe(
        styles["base--hover-has"]
      ));

    it("returns className with 'has-not' if hover is false", () =>
      expect(getHoverClassName("base", false, styles)).toBe(
        styles["base--hover-has-not"]
      ));
  });
});
