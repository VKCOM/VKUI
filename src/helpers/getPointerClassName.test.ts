...
describe(getPointerClassName, () => {
  ...
  describe("with `styles` argument", () => {
    ...
    it("returns 'none' className if there is no mouse", () =>
      expect(getPointerClassName("base", undefined, styles)).toBe(
        styles["base--pointer-none"]
      ));

    it("returns className with 'has' if mouse is true", () =>
      expect(getPointerClassName("base", true, styles)).toBe(
        styles["base--pointer-has"]
      ));

    it("returns className with 'has-not' if mouse is false", () =>
      expect(getPointerClassName("base", false, styles)).toBe(
        styles["base--pointer-has-not"]
      ));
  });
});
