...
describe('Header', () => {
  ...
  it('[typography] HeaderAside is span regardless of platform', () => {
    render(
      <Fragment>
        <ConfigProvider platform={Platform.ANDROID}><Header aside="Русский" /></ConfigProvider>
        <ConfigProvider platform={Platform.IOS}><Header aside="English" /></ConfigProvider>
        <ConfigProvider platform={Platform.VKCOM}><Header aside="Espanõl" /></ConfigProvider>
      </Fragment>,
    );
    expect(getTypographyTagNameByText('Русский')).toMatch('span');
    expect(getTypographyTagNameByText('English')).toMatch('span');
