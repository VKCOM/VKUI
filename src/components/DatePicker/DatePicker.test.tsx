...
describe('DatePicker', () => {
  ...
  describe('custom', () => {
    const forceCustom = (jsx: ReactNode) => <AdaptivityProvider hasMouse>{jsx}</AdaptivityProvider>;
    const getByName = (name: string) => document.getElementsByName(name)[0] as HTMLInputElement;
    const select = (name: string, value: number) => userEvent.selectOptions(getByName(name), String(value));
    ...
    it('fires change', () => {
      let date = getDate();
      render(forceCustom(<DatePicker defaultValue={date} name="date" onDateChange={(v) => date = v} />));
  ...
  describe('native', () => {
    const forceNative = (jsx: ReactNode) => <AdaptivityProvider hasMouse={false}>{jsx}</AdaptivityProvider>;
