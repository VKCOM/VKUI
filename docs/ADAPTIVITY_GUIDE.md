...
окон_ мы используем адаптивность через JS. Для этого создан хук [`useAdaptivityWithMediaQueries()`](../src/hooks/useAdaptivityWithMediaQueries.ts).
...
В нашем примере внутри `<ModalRoot />` и `<ModalPage />` используется `useAdaptivityWithMediaQueries()` для
