...
**new** В 5 версии библиотеки во всех селекторах компонентов появится префикс `vkui` (`.Button` → `.vkuiButton`), чтобы не конфликтовать с другими стилями при встраивании в существующее приложение. Сейчас в компонент для совместимости добавляются классы как с префиксом, так и без. Чтобы отключить классы без префиксов, передайте проп `noLegacyClasses` в `ConfigProvider`.
