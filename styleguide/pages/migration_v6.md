...
Также автоматически больше не проставляются отступы, если внутрь переданы cледующие компоненты: [UserStack](#/UserStack), [FormField](#/FormField).
...
- `ModalCard` или `ModalCardBase` содержит `actions`, а внутрь `children` передан [UsersStack](#/UsersStack). Перед `UserStack` должен быть отступ в `20px`, а между `UserStac` и `actions` отступ должен быть в сумме `24px` (включая отступ по умолчанию `16px`).
