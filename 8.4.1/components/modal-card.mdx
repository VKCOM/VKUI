...
{/* @example-description: Базовая `ModalCard` с управлением через `open/onClose` и действиями подтверждения. */}
<Playground>
...
{/* @example-description: Ручная установка фокуса в `ModalCard` после открытия через обработчик `onOpened`. */}
<Playground>
...
| `descriptionComponent` | `ElementType<any, keyof IntrinsicElements>` | `-` | Позволяет поменять тег используемый для описания. |
...
| `dismissButtonMode` | `"none" \| "inside" \| "outside"` | `-` | Расположение кнопки закрытия (внутри и вне `popout'a`).  Доступно только в `compact`-режиме.  На `iOS` в `regular`-режиме всегда включен `inside`.  ⚠️ ВНИМАНИЕ: использование `none` скрывает крестик, это негативно сказывается на пользовательском опыте. |
...
| `onClose` | `((reason: ModalCardCloseReason, event?: UIEvent<HTMLElement, UIEvent>) => void) \| undefined` | `-` | Будет вызвано при начале закрытия модалки. |
...
| `titleComponent` | `ElementType<any, keyof IntrinsicElements>` | `-` | Позволяет поменять тег используемый для заголовка. |
