...
> Для управления модальными окнами используйте менеджер в виде хука [`useModalManager`](/components/use-modal-manager).
> Данный менеджер остался для обратной совместимости с **v7**. В будущем планируется сделать его приватным.
...
| --- | --- | --- | --- |
| `activeModal` | `ModalRootActiveModal` | `-` | Открывает модальное окно с переданным id. |
...
| `modalOverlayTestId` | `string` | `-` | `data-testid` для маски. |
| `noFocusToDialog` | `boolean` | `-` | Отключает фокус на контейнер диалогового окна при открытии.  > Важно установить фокус после открытия в любое место модалки используя событие `onOpened`, иначе не будет работать закрытие по нажатию `Esc`. |
| `onClose` | `ModalRootCallbackFunction` | `-` | Будет вызвано при начале закрытия активной модалки с её id. |
| `onClosed` | `ModalRootCallbackFunction` | `-` | Будет вызвано при окончательном закрытии активной модалки с её id. |
| `onOpen` | `ModalRootCallbackFunction` | `-` | Будет вызвано при начале открытия активной модалки с её id. |
| `onOpened` | `ModalRootCallbackFunction` | `-` | Будет вызвано при окончательном открытии активной модалки с её id. |
| `onOverlayClosed` | `(() => void)` | `-` |  |
| `onOverlayShowed` | `(() => void)` | `-` |  |
| `usePortal` | `boolean \| HTMLElement \| RefObject<HTMLElement \| null> \| null` | `-` |  |
