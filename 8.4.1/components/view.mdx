...
{/* @example-description: Базовая навигация во `View` с переключением между несколькими `Panel`. */}
...
В iOS можно свайпнуть от левого края назад, чтобы перейти на предыдущий экран. Чтобы повторить такое поведение
в VKUI, нужно:
  ...
  return (
    <Panel id={id}>
      ...
      <Group>
        ...
        <Box padding="system" style={{ height: 50, background: '#eee' }} data-vkui-swipe-back={false}>
