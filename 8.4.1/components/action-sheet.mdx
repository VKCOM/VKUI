...
Компонент выбирает наилучшее расположение самостоятельно, а это свойство задаёт только _приоритетное_ положение. Если для приоритетного положения не хватает места, будет выбрано любое другое подходящее.
...
По умолчанию событие `click` не всплывает — это изолирует компонент от остального приложения. Если нужно обрабатывать это событие (например, есть глобальный обработчик клика для аналитики), используйте свойство `allowClickPropagation`.
...
Задаёт задержку (в миллисекундах) при установке и восстановлении фокуса.

## Кнопка «Отмена» [#cancel-item]

Согласно рекомендациям **Apple**, в `ActionSheet` должен быть вариант действия «Отмена», закрывающий окно без выполнения дополнительных действий.
...
```jsx
<ActionSheetItem href="https://vk.ru" target="_blank">
  Перейти на vk.ru
</ActionSheetItem>
```
...
| `description` | `ReactNode` | `-` | Описание всплывающего окна, под заголовком. |
| `disabled` | `boolean` | `-` | **Deprecated**: Since 7.3.0. Будет удалено в **VKUI v9**. |
| `iosCloseItem` | `ReactNode` | `-` | Только мобильный iOS. |
| `mode` | `"menu" \| "sheet"` | `-` | Режим отображения компонента:  - `sheet` – отображение снизу экрана в виде всплывающего окна, подходит для мобильных устройств - `menu` – отображение в виде всплывающего элемента, относительно якорного элемента. |
| `mount` | `boolean` | `-` | **Deprecated**: Since 7.3.0.  Будет удалено в **VKUI v9**. |
| `onClose` | `((reason: ActionSheetOnCloseReason) => void)` | `-` | Обработчик закрытия всплывающего окна. |
| `**onClosed** \*` | `(options: ActionSheetOnCloseOptions) => void` | `-` | Обработчик закрытия всплывающего окна срабатывающий после завершения анимации закрытия.  > Since 8.0.0 аргумент, переданный в данную функцию, устарел и будет удален в **VKUI v10**. > Для получения причины закрытия всплывающего окна используйте свойство `onClose`. |
| `placement` | `PlacementWithAuto` | `-` | Позиционирование всплывающего окна для десктопа. Компонент выберет наилучшее расположение сам, но можно задать приоритетное направление с помощью этого свойства. |
| `popupOffsetDistance` | `number` | `-` | Отступ, где заданное кол-во единиц равняется пикселям. |
| `restoreFocus` | `boolean \| (() => boolean \| HTMLElement)` | `true` | Управление поведением возврата фокуса при закрытии всплывающего окна. |
| `slotProps` | `{ iosCloseItem?: (Omit<ActionSheetItemProps, "mode" \| "isCancelItem"> & HasRootRef<HTMLElement>); } \| undefined` | `-` | Свойства, которые можно прокинуть внутрь компонента: - `iosCloseItem`: свойства для прокидывания в кнопку отмены на iOS. |
| `timeout` | `number` | `0` | Время в миллисекундах после которого срабатывает автофокус при появлении всплывающего окна. |
| `title` | `ReactNode` | `-` | Заголовок всплывающего окна. |
| `**toggleRef** \*` | `ToggleRef` | `-` | Элемент, рядом с которым вылезает всплывающий элемент на десктопе. Лучше передавать `RefObject` c current. |
