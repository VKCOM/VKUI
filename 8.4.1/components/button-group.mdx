...
{/* @example-description: Базовая горизонтальная группа из нескольких кнопок. */}
<Playground>
  ```jsx
  <ButtonGroup>
...
{/* @example-description: Вертикальная группа с вложенной горизонтальной подгруппой действий. */}
<Playground>
  ```jsx
...
{/* @example-description: Выравнивание растянутой группы кнопок по правому краю контейнера. */}
<Playground>
  ```jsx
...
| --- | --- | --- | --- |
| `align` | `AlignType` | `left` | Горизонтальное выравнивание элементов внутри группы. |
| `gap` | `"s" \| "none" \| "m" \| "space"` | `m` | Выставляет в зависимости от `mode` отступ по вертикали или горизонтали. |
| `getRootRef` | `Ref<HTMLDivElement>` | `-` |  |
| `mode` | `"horizontal" \| "vertical"` | `horizontal` | Задает расположение элементов внутри группы, вертикальное или горизонтальное. |
| `stretched` | `boolean` | `false` | Растягивает компонент на всю ширину контейнера.  Note: Для потомков соответствующее поведение нужно определять самостоятельно, где это необходимо. |
