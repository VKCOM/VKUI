...
{/* @example-description: Базовый `SegmentedControl` с набором текстовых опций и единичным выбором. */}
...
Избегайте слишком большого числа сегментов: рекомендуется от трёх до семи кнопок для планшетов и десктопов и до пяти — в мобильном представлении.
...
{/* @example-description: Использование `SegmentedControl` как `tablist` с управлением связанными `tabpanel`. */}
<Playground Wrapper={BlockWrapper}>
...
return (
  <Group>
    ...
    />
    {selected === 'news' && (
      <Box padding="system" id="tab-content-news" aria-labelledby="tab-news" role="tabpanel" tabIndex={0}>
        Контент новостей
      </Box>
    )}
...
| --- | --- | --- | --- |
| `defaultValue` | `SegmentedControlValue` | `options[0]?.value` | Значение по умолчанию (для неконтролируемого компонента). |
| `getRootRef` | `Ref<HTMLDivElement>` | `-` |  |
| `name` | `string` | `-` | Имя для input-элементов внутри компонента. |
| `onChange` | `((value: T) => void)` | `-` | Обработчик изменения выбранного значения. |
| `**options** \*` | `SegmentedControlOptionInterface<T>[]` | `-` | Массив опций для отображения в компоненте. |
| `size` | `"m" \| "l"` | `l` | Размер компонента. |
| `value` | `SegmentedControlValue` | `-` | Текущее выбранное значение (для контролируемого компонента). |
