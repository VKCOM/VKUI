...
{/* @example-description: Базовые радио-кнопки для выбора одного способа оплаты. */}
<Playground>
  ```jsx
...
> Свойство `value` в данном контексте служит для идентификации поля (например, при передаче в форму). В примере в поле формы "pay" будет значение "card".
...
{/* @example-description: `Radio` в неактивном состоянии `disabled` с предустановленным выбором. */}
<Playground>
  ```jsx
...
{/* @example-description: Кастомизация внутренних элементов `Radio` через `slotProps` и `getRootRef`. */}
<Playground>
  ```jsx
...
{/* @example-description: Использование `Radio` внутри `RadioGroup` с подписью через `FormItem` и `aria-labelledby`. */}
<Playground>
  ```jsx
...
| `description` | `ReactNode` | `-` | Дополнительное описание под основным текстом. |
...
| `labelProps` | `HasDataAttribute` | `-` | **Deprecated**: Since 7.9.0. Вместо этого используйте `slotProps={ root: {...} }`.  Позволяет передавать data-* аттрибуты элементу label. |
...
| `titleAfter` | `ReactNode` | `-` | Элемент после основного текста. |
