...
Мы работаем над качеством библиотеки и подвозим тесты. `yarn test` запускает все возможные тесты, проверяет типы и линтит. `yarn test:unit` — только быстрые юниты. В интерактивном режиме юниты запускают через `yarn test:unit --watch`. Подробности — в нашем [гайде по тестированию](./TESTING.md).
