...
Чтобы понять был это переход вперёд или назад можно воспользоваться хуком `useNavTransitionDirection()`. [Подробнее в примере с компонентом Root](/#/Root?id=useNavTransitionDirection_example).
