...
- В компонент `FormItem` нужно передать `htmlFor`, чтобы корректно связать его с `Input`. <br />
- В дочерний `Input` необходимо передать `id`, который совпадает с `htmlFor` из `FormItem`.
