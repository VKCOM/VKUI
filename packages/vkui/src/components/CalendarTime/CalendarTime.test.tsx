...
describe('CalendarTime', () => {
  it('check onChange should called when select hours and minutes', async () => {
    ...
    const {container} = render(<CalendarTime
      onChange={onChange}
      value={dayDate}
    />)

    const [hourSelect, minuteSelect] = Array.from(container.getElementsByClassName(styles['CalendarTime__picker']))
