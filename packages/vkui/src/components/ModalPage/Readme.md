...
- `ModalPageContent` – оборачивает переданный в `children` в [CustomScrollView](#/CustomScrollView).
...
При разрешении `<= 767px` компонент `ModalPage` имеет вид панели, выдвигающаяся снизу вверх (далее **bottom sheet**).
