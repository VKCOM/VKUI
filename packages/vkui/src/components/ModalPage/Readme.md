...
>
> Чтобы избежать этой проблемы, для первой (нижней) модального окна необходимо отключить захват фокуса с помощью свойства `focusTrapDisabled`.
> Это позволит корректно управлять фокусом во второй (верхней) модальном окне.
