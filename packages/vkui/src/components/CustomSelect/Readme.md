...
- по умолчанию, все data-\* атрибуты попадают на `input` селекта. А значит при передаче `data-testid` или `data-test-id` идентификатор попадёт на `input`.<br />
  Доступ к инпуту может быть полезен для:
  - получения текста, введённого пользователем при поиске опций, в режиме `searchable`;
  - получения информации о наличии фокуса на селекте;
  - симуляции работы с селектом с клавиатуры.
- для получения текста, выбранной в данный момент опции, можно воспользоваться свойством `labelTextTestId`.
- для взаимодействия с кнопкой очистки селекта, которая появляется если селект имеет свойство `searchable` и пользователь выбрал опцию, можно воспользоваться свойством `clearButtonTestId`.
- `CustomSelect` внутри себя хранит невидимый нативный селект, для того, чтобы `CustomSelect` можно было использовать внутри формы. Для получения доступа к этому селекту можно воспользоваться свойством `nativeSelectTestId`. Полезно для доступа к значению `value`, выбранной в данный момент опции.
