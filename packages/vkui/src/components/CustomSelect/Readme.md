...
- для взаимодействия с кнопкой очистки селекта, которая появляется, если селект имеет свойство `searchable` и пользователь выбрал опцию, используйте свойство `clearButtonTestId`.
- `CustomSelect` внутри себя хранит невидимый нативный селект, для того, чтобы `CustomSelect` можно было использовать внутри формы. Для получения доступа к этому селекту используйте свойство `nativeSelectTestId`. Полезно для доступа к значению `value`, выбранной в данный момент опции.
