...
export const Search = ({
  ...
  if (process.env.NODE_ENV === "development" && getRef) {
    warn(
      "Свойство `getRef` устаревшее, используйте `slotProps={ input: { getRootRef: ... } }`"
    );
  }
  ...
  const [controlsAnimationState, controlsAnimationHandlers] =
    useCSSKeyframesAnimationController(hasValue ? "enter" : "exit", {}, true);
  ...
  const onIconClickStart: React.PointerEventHandler<HTMLElement> =
    React.useCallback((e) => onIconClick?.(e), [onIconClick]);

  const onIconCancelClickStart: React.PointerEventHandler<HTMLElement> =
    React.useCallback(
      (e) => {
        e.preventDefault();
  ...
  console.log("controlsAnimationState", controlsAnimationState);
