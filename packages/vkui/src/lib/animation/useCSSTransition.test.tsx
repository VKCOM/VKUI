...
describe(useCSSTransition, () => {
  ...
  describe('first mount', () => {
    it.each([{ state: 'enter' }, { state: 'exit' }])('should mount $state state with transition', ({
      state,
    }) => {
    ...
    it.each([
      false,
      true,
    ])('should mount with enter state but without transition (check callbacks is %s)', (checkCallbacks) => {
      const { result } = renderHook(() =>
    ...
    it.each([
      false,
      true,
    ])('should mount with exit state but  without transition (check callbacks is %s)', (checkCallbacks) => {
      const { result } = renderHook(() =>
  ...
  describe('updates after mount', () => {
    it.each([
      { enableAppear: false },
      { enableAppear: true },
    ])('should enter and exit with transition (%p)', (option) => {
    ...
    it.each([
      { enableAppear: false },
      { enableAppear: true },
    ])('should exit and enter with transition (%p)', (option) => {
      const { result, rerender } = renderHook(
        (inProp) => useCSSTransition<HTMLDivElement>(inProp, { ...option, ...callbacks }),
        { initialProps: true },
      );
  ...
  describe('corner cases', () => {
    it.each([
      { enableExit: true },
      { enableExit: false },
    ])('should exit during appear (%p)', (option) => {
      const { result, rerender } = renderHook(
        (inProp) =>
          useCSSTransition<HTMLDivElement>(inProp, {
            enableAppear: true,
    ...
    it.each([
      { enableExit: true },
      { enableExit: false },
    ])('should exit during enter (%p)', (option) => {
      const { result, rerender } = renderHook(
        (inProp) => useCSSTransition<HTMLDivElement>(inProp, { ...option, ...callbacks }),
        { initialProps: false },
      );
    ...
    it.each([
      { enableEnter: true },
      { enableEnter: false },
    ])('should enter during exit (%p)', (option) => {
      const { result, rerender } = renderHook(
        (inProp) => useCSSTransition<HTMLDivElement>(inProp, { ...option, ...callbacks }),
        { initialProps: true },
      );
