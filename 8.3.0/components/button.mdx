...
⚠️ **Важно для доступности**: При использовании `loading={true}` компонент автоматически устанавливает `aria-label` в значение
`loadingLabel` (по умолчанию "Загрузка..."), чтобы скринридер мог объявить контекст загрузки. Вы можете переопределить это
значение, передав свойство `loadingLabel`.
...
| `appearance` | `"accent" \| "positive" \| "negative" \| "neutral" \| "overlay" \| "accent-invariable"` | `accent` | Цветовая схема кнопки. |
...
| `loadingLabel` | `string` | `Загрузка...` | Текст для `aria-label` при состоянии загрузки. Подменяет переданный в компонент `aria-label` только когда `loading={true}`. |
| `mode` | `"link" \| "primary" \| "secondary" \| "tertiary" \| "outline"` | `primary` | Режим отображения кнопки. |
| `rounded` | `boolean` | `-` | Добавляет скругленные углы кнопке. |
| `size` | `"s" \| "m" \| "l"` | `s` | Размер кнопки. |
| `stretched` | `boolean` | `false` | Растягивает кнопку на всю ширину контейнера. |
