...
{/* @example-description: Базовая `ToolButton` с адаптивными иконками для `compact` и `regular` плотности. */}
...
(см. [Адаптивность | Размеры элементов интерфейса](/overview/adaptivity#density)). Это помогает поддерживать доступность и удобство использования
...
{/* @example-description: Пример `ToolButton` с текстом и направлениями контента `row` и `column`. */}
<Playground style={{ width: 'unset' }}>
  ```jsx
...
{/* @example-description: Сравнение режимов `ToolButton` в `appearance="accent"`. */}
<Playground style={{ width: 'unset' }}>
  ```jsx
...
{/* @example-description: Сравнение режимов `ToolButton` в `appearance="neutral"`. */}
<Playground style={{ width: 'unset' }}>
  ```jsx
...
{/* @example-description: Пример круглой `ToolButton` с `rounded`. */}
<Playground style={{ width: 'unset' }}>
  ```jsx
  <ToolButton
...
| `appearance` | `"accent" \| "neutral"` | `accent` | Внешний вид компонента. |
...
| `direction` | `"row" \| "column"` | `row` | Направление контента. |
...
| `**IconCompact** \*` | `ComponentType<{ className?: string; }>` | `-` | Компонент иконки для компактного размера. |
| `**IconRegular** \*` | `ComponentType<{ className?: string; }>` | `-` | Компонент иконки для обычного размера. |
| `mode` | `"primary" \| "secondary" \| "tertiary" \| "outline"` | `primary` | Режим отображения компонента. |
| `rounded` | `boolean` | `-` | Задаёт `50%` закругления для контейнера.  > Note: игнорируется при `direction="column"` если передан `children`. |
