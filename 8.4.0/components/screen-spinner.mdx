...
{/* @example-description: Базовый `ScreenSpinner` с оверлеем и закрытием в состоянии `cancelable`. */}
<Playground>
...
Чтобы собрать компонент по частям, используйте подкомпоненты. Это даёт гибкость управления отдельными частями и позволяет отвязаться от логики порталов и всплывающих окон (например, при появлении компонента блокируется скролл).

{/* @example-description: Сборка `ScreenSpinner` из подкомпонентов `Container`, `Loader` и `SwapIcon`. */}
<Playground>
  ```jsx
  <ScreenSpinner.Container>
    <ScreenSpinner.Loader />
...
{/* @example-description: Вариант `ScreenSpinner` в режиме `overlay` для контрастного фона. */}
<Playground>
  ```jsx
...
{/* @example-description: Демонстрация состояний `ScreenSpinner`: loading, cancelable, done, error и custom. */}
<Playground>
  ```jsx
  <ScreenSpinner.Container>
    <ScreenSpinner.Loader />
...
| --- | --- | --- | --- |
| `cancelLabel` | `string` | `-` | Используется при `state='cancelable'`. Делает доступным для ассистивных технологий. |
| `customIcon` | `ReactNode` | `-` | Кастомная иконка, работает совместно со `state="custom"`. |
| `disableAnimation` | `boolean` | `-` | Отключение анимации. |
| `getRootRef` | `Ref<HTMLSpanElement>` | `-` |  |
| `label` | `ReactNode` | `-` | Текст под иконкой. |
| `mode` | `"shadow" \| "overlay"` | `-` | Внешний вид отображения спиннера. |
| `noColor` | `boolean` | `-` | Задать цвет можно будет через свойство color родителя. |
| `state` | `"done" \| "loading" \| "custom" \| "error" \| "cancelable"` | `loading` | Тип отображения спиннера. |
...
| `getRootRef` | `Ref<HTMLSpanElement>` | `-` |  |
| `label` | `ReactNode` | `-` | Текст под иконкой. |
| `mode` | `"shadow" \| "overlay"` | `shadow` | Внешний вид отображения спиннера. |
| `state` | `"done" \| "loading" \| "custom" \| "error" \| "cancelable"` | `loading` | Тип отображения спиннера. |
| `visibilityDelay` | `number` | `-` | Задерживает отрисовку элемента на заданное количество миллисекунд. |
