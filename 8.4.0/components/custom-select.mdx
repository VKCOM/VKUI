...
| `onChange` | `((e: ChangeEvent<HTMLSelectElement, Element>, newValue: SelectValue) => void)` | `-` | Обработчик, срабатывающий при изменении выбранного значения. Вторым параметром прокидывается новое значение.  > ⚠️ Лучше использовать второй параметр при работе с компонентом. |
| `onClose` | `VoidFunction` | `-` | Обработчик закрытия выпадающего списка. |
| `onInputChange` | `((e: ChangeEvent<HTMLInputElement, Element>) => void)` | `-` | Событие изменения текстового поля. |
