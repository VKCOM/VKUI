...
{/* @example-description: Базовый счётчик уведомлений в режиме `primary`. */}
<Playground>
  ```jsx
...
{/* @example-description: Цвет `accent` для всех режимов отображения `Counter`. */}
<Playground>
  ```jsx
...
{/* @example-description: Нейтральная цветовая схема `Counter` для разных `mode`. */}
<Playground>
  ```jsx
...
{/* @example-description: Позитивный вариант `Counter` с `appearance="accent-green"`. */}
<Playground>
  ```jsx
...
{/* @example-description: Критический вариант `Counter` с `appearance="accent-red"`. */}
<Playground>
  ```jsx
...
и указать в свойстве `color` нужный цвет. При этом `mode` должен быть отличен от `"inherit"` (значения по умолчанию).
...
{/* @example-description: Пользовательский цвет счётчика через `appearance="custom"` и `color`. */}
<Playground>
  ```jsx
...
{/* @example-description: Примеры использования `Counter` внутри кнопок разных размеров и режимов. */}
<Playground>
  ```jsx
...
| --- | --- | --- | --- |
| `appearance` | `"custom" \| "accent" \| "neutral" \| "accent-green" \| "accent-red"` | `-` | Внешний вид счетчика. |
| `color` | `string` | `-` | Пользовательский цвет (работает только при appearance="custom") - При mode="primary" - изменяет фон - При mode="contrast" - изменяет цвет текста - При mode="tertiary" - изменяет цвет текста - При mode="inherit" - не работает. |
| `getRootRef` | `Ref<HTMLSpanElement>` | `-` |  |
| `mode` | `"inherit" \| "primary" \| "tertiary" \| "contrast"` | `inherit` | Режим отображения счетчика. |
| `size` | `"s" \| "m"` | `m` | Размер счетчика. |
