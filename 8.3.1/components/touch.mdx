...
{/* @example-description: Демонстрация `Touch` с перетаскиванием элемента и обработкой смещений по осям. */}
...
| --- | --- | --- | --- |
| `Component` | `ElementType<any, keyof IntrinsicElements>` | `div` |  |
| `getRootRef` | `Ref<HTMLElement>` | `-` |  |
| `noSlideClick` | `boolean` | `false` | Блокировать click-события после распознавания свайпа. |
...
| `stopPropagation` | `boolean` | `false` | Останавливать всплытие событий. |
| `useCapture` | `boolean` | `false` | Использовать фазу capture для событий. |
