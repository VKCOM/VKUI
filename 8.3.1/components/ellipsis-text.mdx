...
{/* @example-description: Ограничение длинного текста с отображением многоточия при переполнении. */}
<Playground>
  ```jsx
...
{/* @example-description: Использование `EllipsisText` с типографическим компонентом через свойство `Component`. */}
<Playground>
  ```jsx
...
{/* @example-description: Ограничение текста в `HorizontalCell` с явным `maxWidth` для корректного ellipsis. */}
<Playground>
  ```jsx
  <HorizontalCell
    size="auto"
...
| `disableNativeTitle` | `boolean` | `false` | Отключает отображение нативного тултипа с полным текстом. |
| `getRootRef` | `Ref<HTMLElement>` | `-` |  |
| `maxLines` | `number` | `1` | Максимальное количество видимых строк.  > При `maxLines > 1` используется свойство line-clamp, которое поддерживается не всеми версиями браузеров. Используйте с осторожностью > |
| `maxWidth` | `number` | `-` | Пользовательская маскимальная ширина.  Используйте в случаях, когда у контейнера ширина зависит от размера контента, другими словами, когда ширина не ограничена. |
