...
{/* @example-description: Базовая заглушка `Skeleton` для одиночной строки контента. */}
...
{/* @example-description: Комплексный пример `Skeleton` в ячейке и текстовом блоке во время загрузки. */}
<Playground>
  ```jsx
  <Group>
    <SimpleCell
...
{/* @example-description: `Skeleton` без анимации с включенным `noAnimation`. */}
<Playground style={{ maxWidth: 270 }}>
  ```jsx
...
{/* @example-description: Автоматическая высота `Skeleton` внутри типографического компонента `Title`. */}
<Playground style={{ maxWidth: 270 }}>
  ```jsx
...
| --- | --- | --- | --- |
| `colorFrom` | `string` | `-` | Начальный цвет анимации. |
| `colorTo` | `string` | `-` | Финальный цвет анимации. |
| `duration` | `number` | `-` | Длительность анимации в секундах. |
| `getRootRef` | `Ref<HTMLSpanElement \| HTMLDivElement>` | `-` |  |
| `noAnimation` | `boolean` | `false` | Выключает анимацию, в результате чего показывается только один цвет. |
