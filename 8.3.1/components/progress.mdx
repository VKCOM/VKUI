...
| --- | --- | --- | --- |
| `appearance` | `"accent" \| "positive" \| "negative" \| `var(--${string})` \| `#${string}`` | `accent` | Стиль отображения прогрессбара. |
| `getRootRef` | `Ref<HTMLDivElement>` | `-` |  |
| `height` | `number` | `-` | Высота элемента. |
| `trackDisable` | `boolean` | `false` | Сделать прогрессбар прозрачным. |
| `value` | `number` | `0` | Значение прогресса. |
