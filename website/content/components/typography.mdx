...
В сумме 16 токенов для `regular`/`compact` представлений. В разных [темах](/overview/platforms-and-themes#themes)
...
Если вы используете готовые компоненты `VKUI` (например, `SimpleCell`, `Header`) вам не нужно задумываться о
