...
Слоты `Banner` позволяют сохранить его реальную раскладку и заменить заглушками только динамический контент.

{/* @example-description: Заглушка `Banner` с изображением, текстом и действием. */}
