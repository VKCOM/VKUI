...
{/* @example-description: Базовый `Banner` с аватаром, заголовком, подзаголовком и кнопкой действия. */}
<Playground>
  ```jsx
  <Banner
...
{/* @example-description: Баннер в режиме `image` с кастомным фоном и кнопкой поверх изображения. */}
<Playground>
  ```jsx
  <Banner
    mode="image"
...
{/* @example-description: Правая секция `after` в виде `chevron` для обозначения перехода по клику. */}
<Playground>
  ```jsx
  <Banner
...
{/* @example-description: Левая секция `before` с изображением для визуального акцента баннера. */}
<Playground>
  ```jsx
  <Banner
    before={
