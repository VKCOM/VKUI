...
При передаче массива (`[row, column]`) можно задать разные значения для строк (первый элемент) и столбцов (второй элемент).

## Распределение пространства

Компонент равномерно распределяет элементы в контейнере через свойство `columns` или `minColWidth` (они взаимоисключающие).
...
Значение `margin="auto"`/`"auto-inline"`/`"auto-block"` нужно, чтобы визуально выровнять контент в `SimpleGrid` с остальными компонентами VKUI (например, [`Header`](/components/group#header) и [`SimpleCell`](/components/simple-cell)). Поэтому оно обычно актуально только для верхнеуровневых компонентов `SimpleGrid`.

{/* @example-description: Использование `SimpleGrid` c `margin="auto"` для выравнивания внутри `Group`. */}
<Playground>
  ```jsx
  <Group layoutMode="card" header={<Header size="s">Друзья</Header>}>
