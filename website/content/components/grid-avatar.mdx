...
Используйте тот же размер, что и у будущего `GridAvatar`, и круглое скругление. Делить заглушку на сегменты необязательно: во время загрузки состав группы может быть ещё неизвестен.

{/* @example-description: Круглая заглушка для `GridAvatar` размером 48px. */}
