...
const KeyboardBotsOutlineIcon = (
  <AdaptiveIconRenderer
    IconCompact={
      platform === "ios" ? Icon28KeyboardBotsOutline : Icon24KeyboardBotsOutline
    }
...
return (
  <WriteBar
    ...
    inlineAfter={
      <>
        {text.length === 0 && (
          <WriteBarIcon label="Открыть меню бота">
            {KeyboardBotsOutlineIcon}
          </WriteBarIcon>
        )}
        {text.length > 0 && (
          <WriteBarIcon label="Эмоджи и стикеры">
            {SmileOutlineIcon}
          </WriteBarIcon>
        )}
      </>
