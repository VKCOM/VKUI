...
Все боковые элементы рекомендуется оборачивать в `WriteBarIcon`. Также для адптирования размеров иконок стоит использовать
компонент [`AdaptiveIconRenderer`](/components/adaptive-icon-renderer):
