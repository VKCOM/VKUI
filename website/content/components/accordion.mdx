...
Контент монтируется по умолчанию, даже если он не развернут. Такое поведение предусмотрено с учётом серверного рендеринга и SEO.

Если внутри контента находится большая иерархия компонентов или если у вас много аккордеонов, то возможно вам захочется изменить это
поведение, чтобы улучшить производительность. Для этого передайте свойство `unmountOnCollapsed`.
