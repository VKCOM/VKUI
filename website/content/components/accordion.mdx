...
Сохраняйте структуру раскрытого блока: заголовок остаётся в `Accordion.Summary`, а строки содержимого — в `Accordion.Content`.

{/* @example-description: Раскрытый `Accordion` с заглушками заголовка и содержимого. */}
<Playground contrastBackground Wrapper={BlockWrapper} style={{ height: 180 }}>
  ```jsx
  <Accordion expanded>
    <Accordion.Summary>
      <Skeleton width={180} />
...
### Accordion.Icon [#accordion-icon-api]
