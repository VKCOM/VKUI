...
> Нативные свойства `width`/`height` задают размеры самого изображения.
> Чаще всего нужны `size` или `widthSize`/`heightSize` — они задают размеры всего контейнера с изображением.
