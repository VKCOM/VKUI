...
### Отключение заднего фона у конкретного модального окна [#disable-modal-overlay]
...
### Отключение анимации открытия у конкретного модального окна [#disable-animation]

Если вам необходимо отключить анимации для конкретной модалки в `ModalRoot` вы можете сделать так:

```jsx
<ModalRoot activeModal={activeModal} disableAnimation={activeModal === 'modal-1'}>
  <ModalPage id="modal-1" />
  <ModalPage id="modal-2" />
</ModalRoot>
```
