...
Свойство `fetching` подходит, когда поле уже можно показать пользователю, а список опций загружается при открытии или вводе запроса.
...
### Альтернатива: заглушка всего поля

Если до завершения первоначальной загрузки поле ещё нельзя использовать, временно замените его на `SelectMimicry` со `Skeleton` внутри. `SelectMimicry` сохранит внешний вид и адаптивную высоту будущего `CustomSelect`.

{/* @example-description: Заглушка всего поля `CustomSelect` на основе адаптивного `SelectMimicry`. */}
<Playground contrastBackground style={{ maxWidth: 270 }}>

```jsx
<SelectMimicry aria-busy>
  <Skeleton width={120} />
</SelectMimicry>
```
...
`CustomSelect` внутри себя хранит невидимый `<select>`, чтобы его можно было использовать внутри формы.
...
Чтобы уведомить пользователей скринридеров о загрузке списка опций, используйте специальные текстовые метки совместно со свойством `fetching`.
