...
Хук `useSnackbarApi` позволяет получить доступ к API `Snackbar` в приложениях, **обернутых в `SnackbarsManagerProvider`**.
...
> **Важно**:
> Убедитесь, что ваше приложение обернуто в `SnackbarsManagerProvider`, иначе `useSnackbarApi` не будет работать.
