...
- `openCustom(payload: OpenCustomPayload | ComponentType<CustomSnackbarProps>): OpenSnackbarReturn` - Более подробно можно почитать в разделе ["Пользовательские снекбары"](/components/use-snackbar#custom-snackbar).
...
- `setVerticalOffsetBottom(offset: SnackbarsVerticalOffset): void` - Метод для изменения отступа контейнера снекбаров снизу.
