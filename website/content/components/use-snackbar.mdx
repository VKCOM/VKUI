...
- `verticalOffsetYStart: VerticalOffset` - Вертикальный отступ контейнера со снекбарами от верха. Полезно, когда нужно немного сдвинуть верхнюю границу снекбаров.
- `verticalOffsetYEnd: VerticalOffset` - Вертикальный отступ контейнера со снекбарами от низа. Полезно, когда нужно немного сдвинуть нижнюю границу снекбаров.
...
- `open(config: SnackbarConfig): SnackbarApi.OpenSnackbarReturn` — Метод для открытия уведомления. Принимает конфиг, возвращает объект [OpenSnackbarReturn](/components/use-snackbar#open-snackbar-return).
- `openCustom(payload: CustomSnackbar.Payload | ComponentType<CustomSnackbar.Props>): SnackbarApi.OpenSnackbarReturn` - Более подробно можно почитать в разделе ["Пользовательские снекбары"](/components/use-snackbar#custom-snackbar).
- `close(id: string): void` — Метод для закрытия снекбара по `id`.
- `closeAll(): void` — Метод для закрытия всех снекбаров.
- `update(id: string, config: SnackbarApi.UpdateSnackbarProps): void` - Метод для обновления свойств уведомления. Можно поменять любое свойство, кроме 'id' и 'placement'.
...
- `setVerticalOffsetYStart(offset: SnackbarApi.VerticalOffset): void` - Метод для изменения отступа контейнера снекбаров от верха.
- `setVerticalOffsetYEnd(offset: SnackbarApi.VerticalOffset): void` - Метод для изменения отступа контейнера снекбаров снизу.
...
- `update: (props: SnackbarApi.UpdateSnackbarProps) => void` - Метод для изменения свойств созданного снекбара.
...
- **`payload: CustomSnackbar.Payload | ComponentType<CustomSnackbar.Props>`** – принимает либо настройки снекбара в виде объекта типа [CustomSnackbar.Payload](/components/use-snackbar#open-custom-payload),
...
# Хук `useSnackbarApi` [#use-snackbar-api]

Хук `useSnackbarApi` позволяет получить доступ к API Snackbar в приложениях, обернутых в `AppRoot` со свойством `enableSnackbarsController`.

### Пример использования

<Playground Wrapper={BlockWrapper} enableSnackbarsController={true}>
...
> **Важно**:
> Убедитесь, что ваше приложение обернуто в `AppRoot` со свойством `enableSnackbarsController`, иначе `useSnackbarApi` не будет работать.
