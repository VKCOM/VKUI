...
Принимает объект со свойством `maxSnackbarsCount`: Максимальное число открытых на одном `placement` уведомлений. Уведомления, которые не поместились, откроются, когда для них будет место.
...
- `open(config: SnackbarConfig): string` — Метод для открытия уведомления. Принимает конфиг, возвращает `id` снекбара(если передан в конфиг, то вернет его, если нет, то сгенерирует `uuid`).
- `close(id: string): void` — Метод для закрытия снекбара по `id`.
- `closeAll(): void` — Метод для закрытия всех снекбаров.
- `update(id: string, config: Omit<SnackbarConfig, 'id' | 'placement'>): void` - Метод для обновления свойств уведомления. Можно поменять любое свойство, кроме 'id' и 'placement'.
- `getSnackbars(): SnackbarData[]` - Метод для получения списка всех открытых снекбаров.
