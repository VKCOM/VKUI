...
- `open(config: SnackbarConfig): SnackbarApi.OpenSnackbarReturn` — Метод для открытия уведомления. Принимает конфиг, возвращает объект [OpenSnackbarReturn](/components/use-snackbar-manager#open-snackbar-return).
- `openCustom(payload: CustomSnackbar.Payload | ComponentType<CustomSnackbar.Props>): SnackbarApi.OpenSnackbarReturn` - Более подробно можно почитать в разделе ["Пользовательские снекбары"](/components/use-snackbar-manager#custom-snackbar).
