...
**Параметры SnackbarManagerHolder** (все опциональны):

| Параметр        | Тип                  | Описание |
| --------------- | -------------------- | -------- |
| `manager`       | `SnackbarApi.Api`    | Экземпляр менеджера. По умолчанию — глобальный `snackbarManager`. |
| `limit`         | `number`             | Максимум одновременно открытых снекбаров. |
| `queueStrategy` | `"queue" \| "shift"` | Стратегия при переполнении. |
| `offsetYStart`  | `string \| number`   | Отступ контейнера от верха. |
| `offsetYEnd`    | `string \| number`   | Отступ контейнера от низа. |
| `zIndex`        | `string \| number`   | `z-index` контейнера. |
