...
Иногда шапка поля составная — содержит несколько отдельных компонентов. В таком случае используйте подкомпоненты: они позволяют собрать нужный заголовок и гибко управлять свойствами (например, для `a11y`).
...
По умолчанию `bottom` наследует значение `status`. Чтобы задать статус сообщения независимо от статуса поля,
используйте свойство `bottomStatus`. Например, это позволяет показать общую ошибку группы под валидным полем.
...
> Это свойство отвечает только за визуальную составляющую.
> На само поле, обёрнутое в `FormItem`, нужно дополнительно передавать свойство `required`.
