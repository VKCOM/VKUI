...
### Прокрутка жестами на тачпаде

При горизонтальной прокрутке с помощью тачпада (например, на платформе macOS) могут срабатывать жесты переключения страниц назад/вперёд.
Чтобы предотвратить это поведение, используйте свойство `preventOverscroll`:

```jsx
<HorizontalScroll preventOverscroll>{/* контент для прокрутки */}</HorizontalScroll>
```
