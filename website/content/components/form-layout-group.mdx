...
{/* @example-description: Горизонтальная группировка двух полей формы в `FormLayoutGroup`. */}
<Playground>
  ```jsx
...
При недостатке места (например, на мобильных) поля формы сжимаются пропорционально, поэтому в `mode="horizontal"` не рекомендуется располагать более трёх `FormItem`.

{/* @example-description: Вертикальная сегментированная группа связанных полей документа. */}
<Playground>
  ```jsx
...
## Валидация вертикальной сегментированной группы

Передавайте `status="error"` в каждый `FormItem` с невалидным значением. Общее сообщение об ошибке размещайте в свойстве `bottom` последнего `FormItem`: так оно располагается под всей вертикальной сегментированной группой и не разделяет поля.

Если последний `FormItem` сам находится в состоянии `error`, текст в `bottom` получит статус ошибки автоматически. Если ошибка относится к другому полю, а последний `FormItem` валиден, передайте ему `bottomStatus="error"`.

Формулируйте сообщение с учётом количества невалидных полей: «Обязательное поле» для одного и «Обязательные поля» для нескольких. Свяжите сообщение с невалидными полями через `bottomId` и `aria-describedby`.

{/* @example-description: Общая ошибка вертикальной сегментированной группы, в которой два поля невалидны, а последнее содержит сообщение в `bottom`. */}
<Playground>
  ```jsx
  <FormLayoutGroup mode="vertical" segmented>
...
{/* @example-description: Удаляемая горизонтальная группа дат с кнопкой удаления группы. */}
<Playground>
  ```jsx
...
{/* @example-description: Сегментированная горизонтальная группа из `Input` и `Select` для единого e-mail поля. */}
<Playground>
  ```jsx
