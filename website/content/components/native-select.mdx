...
{/* @example-description: Базовый `NativeSelect` с набором нативных опций выбора. */}
...
{/* @example-description: `NativeSelect` в неактивном состоянии `disabled`. */}
<Playground style={{ maxWidth: 280 }}>
  ```jsx
...
{/* @example-description: Валидационные статусы `NativeSelect`: ошибка и успешное состояние. */}
...
{/* @example-description: Кастомизация `NativeSelect` через `slotProps` и доступ к внутреннему `select` по `ref`. */}
<Playground style={{ maxWidth: 280 }}>
  ```jsx
...
До получения опций используйте `SelectMimicry`: он сохраняет внешний вид и адаптивную высоту поля.

{/* @example-description: Заглушка `NativeSelect` на основе адаптивного `SelectMimicry`. */}
<Playground contrastBackground style={{ maxWidth: 280 }}>
  ```jsx
...
Для лучшей доступности оборачивайте `NativeSelect` в `FormItem` — так у поля будут заголовок и описание.
