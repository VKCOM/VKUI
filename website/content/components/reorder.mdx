...
>
> - В качестве `items` можно передать массив элементов типа `string`, `number` или объекта с полями.
>   Если вы используете объекты с полями, рекомендуется использовать свойство `getItemId` для определения корректного `id` элемента.
