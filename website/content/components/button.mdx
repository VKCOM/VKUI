...
Используйте `Skeleton` внутри кнопки: так `Button` сам сохранит корректные высоту, радиус и внутренние отступы для выбранного `size`.

{/* @example-description: Кнопка размера `m` с заглушкой вместо текста. */}
<Playground contrastBackground>
  ```jsx
