...
Принимает все свойства компонента [`ModalRoot`](/components/modal-root) кроме `activeModal` и `children`. А также принимает свойство `saveHistory`, которое отвечает
за то, будет ли сохраняться история открытия модальных окон.
...
- `openModalCard: (props: OpenModalCardProps) => OpenModalReturn` - Метод для открытия `ModalCard`, принимает свойства `ModalCard`. Возвращает объект типа [`OpenModalReturn`](/components/use-modal-root#open-modal-return).
- `openModalPage: (props: OpenModalPageProps) => OpenModalReturn` - Метод для открытия `ModalPage`, принимает свойства `ModalPage`. Возвращает объект типа [`OpenModalReturn`](/components/use-modal-root#open-modal-return).
- `openCustomModalPage: (props: CustomModalPayload) => OpenModalReturn` - Метод для открытия пользовательского модального окна на базе `ModalPage`. Более подробно можно почитать в разделе ["Пользовательские модальные окна"](/components/use-modal-root#custom-modals).
- `openCustomModalCard: (props: CustomModalPayload) => OpenModalReturn` - Метод для открытия пользовательского модального окна на базе `ModalCard`. Более подробно можно почитать в разделе ["Пользовательские модальные окна"](/components/use-modal-root#custom-modals).
...
- `close: (id: string) => void` - Метод для закрытия определенного модального окна. Принимает id модального окна.
- `closeAll: () => void` - Метод для закрытия всех созданных модальных окон.
- `setSaveHistory: (value: boolean) => void` - Метод для изменения значения флага `saveHistory`.
...
- `onClose: <R>(resolve?: () => R, reject?: VoidFunction) => Promise<R>` - Метод, который возвращает `Promise`, который разрешится, когда модальное окно будет закрыто.
...
При использовании хука `useModalRoot` история открытых модальных окон сохраняется по умолчанию.
Чтобы отключить это поведение вы можете:

- прокинуть в `useModalRoot` свойство `saveHistory`;
- использовать метод `setSaveHistory` в api.
...
const openModal = (type: 'card' | 'page') => {
  if (type === 'card') {
    const { close } = api.openModalCard({
      ...
      actions: (
        <ButtonGroup stretched mode="vertical">
          <Button size="l" mode="primary" stretched onClick={() => openModal('page')}>
            Открыть ModalPage
          </Button>
          <Button size="l" mode="primary" stretched onClick={() => openModal('card')}>
            Открыть ModalCard
          </Button>
    ...
    const { close } = api.openModalPage({
      ...
      children: (
        <Group>
          <CellButton onClick={() => openModal('page')}>Open ModalPage</CellButton>
          <CellButton onClick={() => openModal('card')}>Open ModalCard</CellButton>
        </Group>
      ),
    });
  }
...
Для этого в `useModalRoot` есть методы **`openCustomModalCard`** и **`openCustomModalPage`**, который позволяет отрисовать модальное окно произвольного вида на базе `ModalCard` или `ModalPage`.
...
- **`baseProps?: BaseProps`** – базовые свойства, которые будут проброшены в компонент модалки;
...
2. Через `api.openCustomModalCard({ component, props })` вы «регистрируете» это окно и открываете его.
...
Если вы хотите показать модальное окно, не прокидывая ничего в него снаружи, то можете в качестве параметра в `openCustomModalPage`/`openCustomModalCard` прокинуть саму функцию отрисовки компонента.
...
# useModalsApi [#use-modals-api]

Хук для еще большего упрощения работы с модальными окнами.

> **Важно**: Для его работы необходимо, чтобы ваше приложение было обернуто в [`AppRoot`](/components/app-root) со свойством `enableModalsController`.
...
<Playground enableModalsController={true}>
