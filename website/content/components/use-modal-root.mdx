...
- `openCustomModal: OpenModal` - Метод для открытия пользовательского модального окна. Более подробео можно почитать в разделе ["Пользовательские модальные окна"](/components/use-modal-root#custom-modals)
...
- **`props: CustomModalProps`** – настройки модального окна (см. ниже).

### `CustomModalProps` [#custom-modal-props]

`CustomModalProps` описывает свойства вашего модального окна.
...
#### Свойства `CustomModalProps`:

- **`id?: string`** – идентификатор окна (если не указать, будет сгенерирован автоматически);
- **`props?: BaseProps & AdditionalProps`** – объединение базовых и дополнительных свойств, которые будут проброшены в компонент модалки;
- **`component: ReactComponent<OpenModalComponentsProps<BaseProps, AdditionalProps>>`** – React-компонент, который будет рендерить содержимое окна.

В компонент автоматически передаются:

- все базовые свойства модального окна,
...
3. При необходимости можно передавать **дополнительные props** и управлять логикой изнутри (например, открывать следующую модалку или обновлять заголовок).
