...
В `v8` обновлены минимальные версии поддерживаемых браузеров. Ниже показана разница с `v7`, чтобы было наглядно видно, какие версии больше не поддерживаются:
...
### События, `event.currentTarget` и типы колбэков

Раньше многие события (например, `onClick`) обрабатывались на внутренних элементах (`input`, контейнер
контента и т.п.), поэтому `event.currentTarget` указывал именно на них. В v8 большая часть обработчиков,
переданных через `restProps`, вешается на корневой элемент компонента, а события от внутренних элементов
доходят до него по всплытию.

> ⚠️ Если в логике обработчика вы полагались на то, что `event.currentTarget` — это внутренний элемент
> (`HTMLInputElement`, внутренний контейнер и т.п.), теперь `event.currentTarget` будет ссылаться на корень
> компонента (например, на `<label>` у `Switch`, `Checkbox`, `Radio` или на обёртку у `CustomSelect`).
...
Такой подход применим и к другим компонентам из раздела ниже: вместо передачи обработчиков напрямую в
компонент используйте нужный слот (`slotProps.input`, `slotProps.textarea`, `slotProps.content` и т.д.),
если ваша логика должна работать именно с внутренними элементами.
