...
>
> Воспользуйтесь инструментом автоматической миграции для TypeScript-файлов:
>
> 1. Обновите `@vkontakte/vkui` до **v8**
> 2. В директории с исходным кодом (обычно `src/`) запустите `npx @vkontakte/vkui-codemods`.
> 3. Простые изменения сделает codemod, о сложных сообщит в консоли.
>
...
> ⚠️ Свойство `getRef` в компонентах ниже помечено как `@deprecated`. Используйте `slotProps` с полем `getRootRef` для ссылки на внутренний элемент.
...
Компонент `ModalRoot` помечен как `@deprecated`. Для императивного управления модальными окнами используйте хук [useModalManager](/components/use-modal-manager).
...
Компонент `FixedLayout` помечен как `@deprecated` и будет удалён в **v10**. Используйте [Box](/components/box) со свойствами `position="sticky"` и `insetBlockStart`/`insetBlockEnd`.
...
> `<FixedLayout vertical="bottom" />` на `Box` важно сохранить порядок элемента в документе.
