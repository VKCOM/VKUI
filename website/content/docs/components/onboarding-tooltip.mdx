...
> Для показа всплывающей подсказки по нажатию или наведению воспользуйтесь [`Tooltip`](#/Tooltip).
...
Чтобы использовать компонента без [`Panel`](#/Panel), [`PanelHeader`](#/PanelHeader) или [`FixedLayout`](#/FixedLayout):
