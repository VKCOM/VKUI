...
Аватарки можно оборачивать в кастомный компонент для добавления дополнительного функционала с помощью свойства `renderWrapper` у элемента списка `photos`:
...
1. Максимальное число в счётчике: 99
2. При `size="s"` счётчик не отображается
3. Не используйте с `avatarsPosition="block-start"` и `size="l"`
