...
- `"default"` — стандартный цвет отображения текста, используется в действиях,
  где нет необходимости в привлечении дополнительного внимания;
- `"destructive"` — цвет критических действий (чаще всего красный), старайтесь располагать такие действия сверху списка,
...
Подробнее с использованием можно ознакомиться на странице [`ActionSheet`](/docs/components/action-sheet#cancel-item).

```jsx
<ActionSheetItem mode="cancel" isCancelItem>
  Отмена
</ActionSheetItem>
```
...
import { ActionSheetItem, AdaptiveIconRenderer } from '@vkontakte/vkui';

<ActionSheetItem
  before={<AdaptiveIconRenderer IconCompact={Icon20WriteOutline} IconRegular={Icon28EditOutline} />}
>
...
<ActionSheet title="Изменить скорость видео" onClose={onClose} toggleRef={selectableTargetRef}>
  {['0.25x', '0.5x', '0.75x', 'Нормальная', '1.25x', '1.5x', '2x', '3x'].map((speed) => (
    <ActionSheetItem
      onChange={onChange}
...
```jsx
import { Icon20CheckBoxOn, Icon24CheckBoxOn } from '@vkontakte/icons';
import { ActionSheetItem, AdaptiveIconRenderer } from '@vkontakte/vkui';
