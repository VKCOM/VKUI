...
[Обязательный компонент](/docs/overview/usage#required-wrapper), в который нужно обернуть всё приложение.
...
> с помощью [`PlatformProvider`](#/PlatformProvider), [`ColorSchemeProvider`](#/ColorSchemeProvider),
> [`LocaleProvider`](#/LocaleProvider) или [`DirectionProvider`](#/DirectionProvider).
