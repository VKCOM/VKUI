...
Для кастомизации стрелок навигации
...
- `showArrows` - флаг, определяющий нужно ли показывать стрелки. Видимость стрелок также зависит от параметра адаптивности `hasPointer`. При `hasPointer={false}` стрелки отображаться не будут.
