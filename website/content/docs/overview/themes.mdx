...
задать поддержку только одной через свойство `platform` у [`ConfigProvider`](/docs/components/config-provider):
...
- `android` (используется по умолчанию) или `material`
...
[`ColorSchemeProvider`](/docs/components/color-scheme-provider). Убедитесь, что потомок `ColorSchemeProvider`
