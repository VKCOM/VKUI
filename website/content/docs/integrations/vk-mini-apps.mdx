...
необходимо использовать компонент [`PanelHeader`](/docs/components/panel-header) на каждом
экране приложения. Он должен содержать название приложения и значок **Назад** (см.
[`PanelHeaderBack`](/docs/components/panel-header-button#panelheaderback)) на тех экранах, где тот
требуется.
...
Чтобы автоматически скрывать `after` у `PanelHeader`, в [`ConfigProvider`](/docs/components/config-provider) есть параметр `hasCustomPanelHeaderAfter`.
...
Виброотликом можно воспользоваться, например, при использовании компонента [`PullToRefresh`](/docs/components/pull-to-refresh)
на `onRefresh`.
