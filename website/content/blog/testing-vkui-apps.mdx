...
## Тестирование с помощью Vitest и React Testing Library

Несмотря на то, что существует несколько различных инструментов по тестированию React-приложений,
мы рекомендуем использовать `Vitest` и `React Testing Library` для функционального или unit-тестирования VKUI-приложений.
...
<Callout>
  ...
  не так, то обратитесь к документации [Vitest](https://vitest.dev) и [React Testing
...
Специфичной настройки `Vitest` для тестирования VKUI-компонентов не требуется, но могут возникнуть трудности
...
Некоторые компоненты зависят от браузерного API, которое не реализовано в окружении `Vitest`, поэтому для
тестирования подобных компонентов вам необходимо предоставить `mock`-реализацию самостоятельно.
Подробнее читайте в [этом руководстве](https://vitest.dev/guide/mocking.html).
...
<Callout>
  ...
  вместо этого импортируется обёртка, которая адаптирует функцию под работу в `Vitest`-окружении:
 ...
 */
export const userEvent = userEventLib.setup({ advanceTimers: vi.advanceTimersByTime });
```
