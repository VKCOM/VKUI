...
в структуре. Например, это справедливо для компонента `Select`, который на мобильных устройствах (Android и iOS)
...
Чтобы избежать ошибок, проще всего обернуть `Select` в `PlatformProvider` с указанием необходимой платформы

```jsx
import { render } from '@testing-library/react';
import { PlatformProvider, Select } from '@vkontakte/vkui';

test('test vkcom view', () => {
  render(
...
test('test material view', () => {
  render(
    <PlatformProvider value="material">
      <Select id="select-id" options={[]} {...props} />
    </PlatformProvider>,
  );
...
test('test is value selected on vkcom', () => {
  render(
    <PlatformProvider value="vkcom">
      <Select
