...
[VK Mini Apps](https://vk.ru/miniapps) — кроссплатформенные мини-приложения в экосистеме [ВКонтакте](https://vk.ru/). VKUI изначально задумывался как набор компонентов для таких приложений. Со временем библиотека вышла за эти рамки, но по-прежнему отлично подходит для мини-приложений. Ниже — рекомендации по интеграции.

</Overview>

<Callout>
  О разработке **VK Mini Apps** — в [статье «Мини-приложения» на портале VK для разработчиков](https://dev.vk.ru/mini-apps/overview).
</Callout>

## Конфигурация VKUI

<Callout>
  Рекомендуем шаблон [`@vkontakte/create-vk-mini-app`](https://dev.vk.ru/ru/mini-apps/getting-started/create-vk-mini-app) — в нём описанная ниже конфигурация есть по умолчанию.
</Callout>
...
Далее подписываемся на изменения из **VK Bridge** и передаём их в `ConfigProvider` из VKUI. Используем готовые хуки из [`@vkontakte/vk-bridge-react`](https://www.npmjs.com/package/@vkontakte/vk-bridge).
...
const App = () => {
  ...
  const { vk_platform } = parseURLSearchParamsForGetLaunchParams(window.location.search); // [опционально] Платформа может передаваться через URL (см. https://dev.vk.ru/mini-apps/development/launch-params#vk_platform)
...
- В _стандартном_ мини-приложении ВКонтакте при переходах отправляйте [`VKWebAppSetSwipeSettings`](https://dev.vk.ru/bridge/VKWebAppSetSwipeSettings) с `history: true` на первой панели и `history: false` на остальных.
- Во _внутреннем_ мини-приложении ВКонтакте отправляйте [`VKWebAppEnableSwipeBack`](https://dev.vk.ru/bridge/VKWebAppEnableSwipeBack) при переходе на первый экран и [`VKWebAppDisableSwipeBack`](https://dev.vk.ru/bridge/VKWebAppDisableSwipeBack) — на любой другой.
...
<Callout>
На Android нажатие кнопки **Назад** вызывает в WebView событие [history.back](https://developer.mozilla.org/ru/docs/Web/API/Window/history). Официальное приложение вернётся на предыдущую страницу вашего приложения или закроет его, если вернуться некуда. Поэтому для корректной навигации обрабатывайте нажатие аппаратной клавиши в мини-приложении и реализуйте роутинг — например, через `react-router`.

</Callout>

### `PanelHeader`

Мини-приложению доступна почти вся площадь экрана, поэтому для корректной навигации используйте [`PanelHeader`](/components/panel-header) на каждом экране. Он должен содержать название приложения и значок **Назад** (см. [`PanelHeaderBack`](/components/panel-header#panel-header-back)) там, где он нужен.

<Callout>
_Правый верхний угол_ `PanelHeader` зарезервирован под нативный бар с кнопками управления мини-приложением, поэтому параметр `after` использовать не рекомендуется.
...
[@vkontakte/vk-bridge-react](https://www.npmjs.com/package/@vkontakte/vk-bridge) предоставляет функцию `runTapticImpactOccurred`, которая отправляет событие [VKWebAppTapticImpactOccurred](https://dev.vk.ru/bridge/VKWebAppTapticImpactOccurred) и заранее проверяет, что вибрация доступна на устройстве.

Виброотклик удобно использовать, например, в `onRefresh` у [`PullToRefresh`](/components/pull-to-refresh).
