...
> Прежде чем начинать организовывать навигационную структуру, следует обязательно обернуть ваше приложение
> в [`SplitLayout`](/components/split-layout) с хотя бы одним [`SplitCol`](/components/split-layout#split-col) – это обеспечит
> безопасные боковые отступы и корректную работу анимаций.

Так так эта обёртка должна быть одна на всё приложение, её достаточно разместить ближе к корню — в точке входа приложения.
...
<Playground Wrapper={FixedLayoutWrapper} style={{ minHeight: 440 }}>
...
const SettingsScenario = ({ id, goBack }) => {
  ...
  return (
    <View id={id} activePanel={activePanel}>
      <Panel disableBackground={platform === 'vkcom'} id="notification-main-panel">
      ...
      <Panel disableBackground={platform === 'vkcom'} id="notification-messenger-panel">
        <PanelHeader
...
const AboutScenario = ({ id, goBack }) => {
  ...
  return (
    <View id={id} activePanel="about-version-panel">
      <Panel disableBackground={platform === 'vkcom'} id="about-version-panel">
  ...
  return (
    <Root id={id} activeView={activeScenario}>
      <View id="main-scenario" activePanel="main-menu-panel">
        <Panel disableBackground={platform === 'vkcom'} id="main-menu-panel">
  ...
  <SplitLayout header={platform !== 'vkcom' && <PanelHeader delimiter="none" />} center>
    {viewWidth.tabletPlus && (
      <SplitCol className={viewWidth.tabletPlus.className} width={280} maxWidth={280} fixed>
        <Panel disableBackground={platform === 'vkcom'}>
            ...
            default:
              return (
                <View key={id} id={id} activePanel={`${id}-panel`}>
                  <Panel disableBackground={platform === 'vkcom'} id={`${id}-panel`}>
                    <PanelHeader>{label}</PanelHeader>
                    <Group>
