...
безопасные боковые отступы.

Так так эта обёртка должна быть одна на всё приложение, его достаточно разместить ближе к корню — в точке входа приложения.
...
```tsx showLineNumbers filename="src/panels/home.tsx" {4-6}
...
```tsx filename="src/scenarios.tsx"
import { useState } from 'react';
